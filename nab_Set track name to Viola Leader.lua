...
function rename_selected_tracks()
    ...
    reaper.GetSetMediaTrackInfo_String(track, "P_NAME", "Viola Leader", true)
    ...
    if num_tracks > 1 then
        for i = 1, num_tracks do
            ...
            if name:match("^Viola Leader") then
                local new_name = name:gsub("^Viola Leader", "Viola Leader ")
                local number = tonumber(new_name:match("%d+$")) or 0
                reaper.GetSetMediaTrackInfo_String(track, "P_NAME", new_name .. " " .. number+1, true)
            else
                reaper.GetSetMediaTrackInfo_String(track, "P_NAME", "Viola Leader " .. i, true)
            end
        end
    end
