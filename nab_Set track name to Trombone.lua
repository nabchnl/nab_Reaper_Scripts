...
function rename_selected_tracks()
    ...
    reaper.GetSetMediaTrackInfo_String(track, "P_NAME", "Trombone", true)
    ...
    if num_tracks > 1 then
        for i = 1, num_tracks do
            ...
            if name:match("^Trombone") then
                local new_name = name:gsub("^Trombone", "Trombone ")
                local number = tonumber(new_name:match("%d+$")) or 0
                reaper.GetSetMediaTrackInfo_String(track, "P_NAME", new_name .. " " .. number+1, true)
            else
                reaper.GetSetMediaTrackInfo_String(track, "P_NAME", "Trombone " .. i, true)
            end
        end
    end
